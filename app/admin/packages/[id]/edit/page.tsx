...
export default function EditPackagePage() {
  ...
  useEffect(() => {
    async function load() {
      const pkg = await fetchSinglePackage(id);
      if (!pkg) {
        setNotFound(true);
        setLoading(false);
        return;
      ...
      // Match the package's stored category to a known option. If it
      // doesn't match anything (e.g. legacy/free-typed category data),
      // fall back to the first option rather than silently keeping a
      // broken value — the admin will need to explicitly re-pick it.
      const matched = CATEGORY_OPTIONS.find((opt) => opt.slug === pkg.category);
      setCategory(matched || CATEGORY_OPTIONS[2]);

      setItems(
        pkg.items && pkg.items.length > 0
          ? pkg.items.map((it) => ({ ...it, id: it.id || newItemId() }))
  ...
  const updateItem = (itemId: string, field: keyof PackageItem, value: string | number) => {
    setItems((prev) =>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    const ok = await updatePackage(id, {
      title: title.trim(),
      slug: slugify(title),
  ...
  if (notFound) {
    return (
      <div className="max-w-3xl">
        <p className="text-sm text-neutral-500 mb-4">Package not found.</p>
        <Link
          href="/admin/packages"
  ...
  return (
    <div className="max-w-3xl">
      <Link
        href="/admin/packages"
        className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-neutral-400 hover:text-burgundy mb-4"
      >
        <ArrowLeft className="w-4 h-4" /> Back to Packages
      </Link>

      <h1 className="font-serif text-2xl font-bold text-charcoal mb-6">
        Edit Special Package
      </h1>
